...
namespace EcommerceMVC.Controllers
{
    public class CartController : Controller
    {
        ...
        {
            // Lấy mã khách hàng đang đăng nhập (nếu có)
            var customerId = HttpContext.User?.Claims
                .SingleOrDefault(p => p.Type == MySetting.CLAIM_CUSTOMERID)?.Value;

            // ===== 1. Nếu tick "Same infor customer?" thì fill thông tin từ KhachHang =====
            if (model.GiongKhachHang && !string.IsNullOrEmpty(customerId))
            {
                var khachHangDb = _db.KhachHangs.SingleOrDefault(kh => kh.MaKh == customerId);
                if (khachHangDb != null)
                {
                    if (string.IsNullOrWhiteSpace(model.HoTen))
                        model.HoTen = khachHangDb.HoTen;

                    if (string.IsNullOrWhiteSpace(model.DiaChi))
                        model.DiaChi = khachHangDb.DiaChi;

                    if (string.IsNullOrWhiteSpace(model.DienThoai))
                        model.DienThoai = khachHangDb.DienThoai;
                }

                // Nếu bạn có [Required] (trong tương lai) thì mới cần re-validate
                ModelState.Clear();
                TryValidateModel(model);
            }

            // ===== 2. Nếu có validation nào khác fail thì quay lại =====
            if (!ModelState.IsValid)
                return View(GetCart()); // View đang dùng @model IEnumerable<CartItem>

            // ===== 3. Nếu bấm VNPAY -> tạo URL & redirect =====
            ...
            // ===== 4. Nếu không phải VNPAY -> xử lý COD như cũ =====
